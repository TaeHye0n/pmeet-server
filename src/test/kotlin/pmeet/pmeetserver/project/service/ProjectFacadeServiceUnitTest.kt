...
internal class ProjectFacadeServiceUnitTest : DescribeSpec({
  ...
  describe("getMyProjectSliceInProgress") {
    context("userId와 Pageable이 주어지면") {
      ...
      coEvery {
        projectService.getProjectsByProjectMemberUserIdAndIsCompletedOrderByCreatedAtDesc(
          userId,
          false,
          pageable
        )
      ...
      it("내가 속한 진행 중인 프로젝트 정보와 다른 참여자들 정보를 반환한다") {
        runTest {
          val responseDtos = projectFacadeService.getMyProjectSliceInProgress(
            userId,
            pageable
          )
